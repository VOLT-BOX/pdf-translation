...
def query_job(*, token: str, job_id: str, base_url: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        # 云端 job 不存在/已被清理(如处理卡死后被回收):立即失败,不要傻等到
        # poll_timeout(1800s)才退出——否则任务会"卡 running"半个钟。
        if err.response is not None and err.response.status_code == 404:
            _body = ""
            try:
                _body = (err.response.text or "")[:500]
            except Exception:  # noqa: BLE001
                _body = "<unreadable>"
            raise PaddleNetworkError(
                f"Paddle OCR job {job_id} 不存在/已被云端清理(可能处理失败或超时被回收): {_body}"
            ) from err
        raise
