...
def is_forced_table_item(item: dict | None) -> bool:
    """enable_table_translation 强制翻译的表格项。

    表格块只在 enable_table_translation=True 时才会进翻译 payload
    (json_extractor 对 table 块无条件丢弃)。因此 block_kind=="table" 即等价于
    "该表格被强制要求翻译":policy/校验/规范化层一律不得把它判成 keep_origin,
    也不得走 fast-path 跳过——否则 _force_table_translation 的标记会被
    policy_translate=False(provider_non_text:table)与 raw_block_type=table
    的 keep_origin 规则覆盖,表格永远轮不到调用模型。
    """
    return item_block_kind(item) == "table"
