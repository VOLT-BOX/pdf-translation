...
def should_reject_keep_origin(item: dict, decision: str, payload: dict[str, str] | None = None) -> bool:
    ...
    if payload and is_internal_placeholder_degraded(payload):
        return False
    # enable_table_translation 强制翻译的表格:任何来源的 keep_origin 都要拒绝。
    # 命中后上层(batch 路径)会走部分重试→单条 plain-text 路径强制 translate,
...
def _review_translated_text(
    ...
    # 续写组(continuation group)成员块的 source_text 是整组拼接文本,
    # 而 translated_text 只是切给该成员的一段 —— 分母是整组、分子是单成员,
    # 长度比必然偏低,会机械误判为 truncated。整组翻译质量由组级别的校验把关,
    # 这里对成员块跳过 truncation 检查。
    if not _is_continuation_item(item):
